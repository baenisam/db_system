...
const Ventes = props => {
  ...
  const getProduts = data => {
    setLoading(true);
    axios({
      url: config.BASE_URL + `facture/${en && en.id_entreprise}/load/${data}/1`,
      method: 'GET',
      headers: {
      ...
      <View
        ...
        <View
          ...
          <TouchableOpacity
            ...
            <Icons.FontAwesome name="arrow-left" color={COLORS.white} size={15} />
          </TouchableOpacity>
          <Text style={{color: COLORS.white, ...styles.title}}>Ventes</Text>
          <TouchableOpacity
