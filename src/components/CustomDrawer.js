...
const CustomDrawer = props => {
  const {mode, toogleTheme,en,getEntreprise, Logout,allEntreprise,toogleSound, sound,ShoosedEntreprise, userData, isLoading} = React.useContext(GlobalContext);
  ...
  return (
    <View style={{flex: 1, backgroundColor: COLORS.headerWhite}}> 
      ...
      <DrawerContentScrollView showsVerticalScrollIndicator={false} style={{}} {...props}>
        ...
        <View
          style={{
            ...
            <TouchableOpacity onPress={() => getEntreprise()}>
              <Icons.FontAwesome name="refresh" color={COLORS.txtblack} size={20}/>
            </TouchableOpacity>
        </View>
        {isLoading && <ActivityIndicator style={{marginTop:10}} size={'small'} color={COLORS.txtblack}/>}
        ...
        <View
          style={{
            ...
            style={{
              ...
              <Icons.MaterialIcons name="business-center" size={18} color={COLORS.txtblack}/>
            <Text style={{
               fontFamily: 'Poppins-Medium',
               color: COLORS.txtblack,
               marginLeft: 5,
               fontSize: 15,
            }}>{item.nom_entreprise}</Text>
            </TouchableOpacity>
